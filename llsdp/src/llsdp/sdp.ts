...
import {
  ...
  SDPMode,
  LENIENT_FLAGS,
  MAJOR, METHOD_MAP, METHODS, METHODS_SDP, METHODS_ICE, METHODS_RTSP,
...
export class SDP {
  ...
  constructor(private readonly llparse: LLParse,
              private readonly mode: SDPMode = 'loose') {
    const p = llparse;
    ...
    this.span = {
      body: p.span(p.code.span('llsdp__on_body')),
      headerField: p.span(p.code.span('llsdp__on_header_field')),
      headerValue: p.span(p.code.span('llsdp__on_header_value')),
      status: p.span(p.code.span('llsdp__on_status')),
    };

    /* tslint:disable:object-literal-sort-keys */
    this.callback = {
      // User callbacks
      onUrlComplete: p.code.match('llsdp__on_url_complete'),
      onStatusComplete: p.code.match('llsdp__on_status_complete'),
      onHeaderFieldComplete: p.code.match('llsdp__on_header_field_complete'),
      onHeaderValueComplete: p.code.match('llsdp__on_header_value_complete'),
      onHeadersComplete: p.code.match('llsdp__on_headers_complete'),
      onMessageBegin: p.code.match('llsdp__on_message_begin'),
	...
	onSessionDescriptionEntryComplete: p.code.match('llsdp__on_session_description_entry_complete'),
      ...
      afterHeadersComplete: p.code.match('llsdp__after_headers_complete'),
      afterMessageComplete: p.code.match('llsdp__after_message_complete'),
    };
  ...
  public build(): ISDPResult {
    const p = this.llparse;

    p.property('i64', 'content_length');
    p.property('i8', 'type');
    p.property('i8', 'method');
    p.property('i8', 'sdp_major');
    p.property('i8', 'sdp_minor');
  ...
  private buildLine(): void {
    ...
    n('req_or_res_method')
      ...
      .match('SDP/', this.update('type', TYPE.RESPONSE, 'res_sdp_major'))
      .otherwise(p.error(ERROR.INVALID_CONSTANT, 'Invalid word encountered'));

    // Response

    n('start_res')
      .match('SDP/', n('res_sdp_major'))
      .otherwise(p.error(ERROR.INVALID_CONSTANT, 'Expected SDP/'));

    n('res_sdp_major')
      .select(MAJOR, this.store('sdp_major', 'res_sdp_dot'))
      .otherwise(p.error(ERROR.INVALID_VERSION, 'Invalid major version'));

    n('res_sdp_dot')
      .match('.', n('res_sdp_minor'))
      .otherwise(p.error(ERROR.INVALID_VERSION, 'Expected dot'));

    n('res_sdp_minor')
      .select(MINOR, this.store('sdp_minor', 'res_sdp_end'))
    ...
    const onUrlCompleteSDP = p.invoke(this.callback.onUrlComplete);
    onUrlCompleteSDP.otherwise(n('req_sdp_start'));

    url.exit.toSDP
      .otherwise(onUrlCompleteSDP);

    const onUrlCompleteSDP09 = p.invoke(this.callback.onUrlComplete);
    onUrlCompleteSDP09.otherwise(n('header_field_start'));

    url.exit.toSDP09
      .otherwise(
        this.update('sdp_major', 0,
          this.update('sdp_minor', 9, onUrlCompleteSDP09)),
      );

    const checkMethod = (methods: METHODS[], error: string): Node => {
      const success = n('req_sdp_major');
    ...
    n('req_sdp_start')
      .match('SDP/', checkMethod(METHODS_SDP,
        'Invalid method for SDP/x.x request'))
      ...
      .match(' ', n('req_sdp_start'))
      .otherwise(p.error(ERROR.INVALID_CONSTANT, 'Expected SDP/'));

    n('req_sdp_major')
      .select(MAJOR, this.store('sdp_major', 'req_sdp_dot'))
      .otherwise(p.error(ERROR.INVALID_VERSION, 'Invalid major version'));

    n('req_sdp_dot')
      .match('.', n('req_sdp_minor'))
      .otherwise(p.error(ERROR.INVALID_VERSION, 'Expected dot'));

    n('req_sdp_minor')
      .select(MINOR, this.store('sdp_minor', 'req_sdp_end'))
      .otherwise(p.error(ERROR.INVALID_VERSION, 'Invalid minor version'));

    n('req_sdp_end').otherwise(this.load('method', {
    ...
    n('req_pri_upgrade')
      ...
      .otherwise(
        p.error(ERROR.INVALID_VERSION, 'Expected SDP/2 Connection Preface'));
  }
  ...
  private buildHeaderField(): void {
    ...
    private buildSessionDescription(): void {
	const p = this.lparse;
	const n = (name: string): Match => this.node<Match>(name);
  ...
  private buildHeaderValue(): void {
    ...
    //
    // See: sdps://tools.ietf.org/html/rfc7230#section-3.2.2
     ...
     *
     * sdps://www.rfc-editor.org/rfc/rfc7230.html#section-3.3.3
     *
    ...
    const afterHeadersComplete = p.invoke(callback.afterHeadersComplete, {
      ...
      // non-chunked `Transfer-Encoding` for request, see `src/native/sdp.c`
