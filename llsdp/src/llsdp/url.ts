...
export class URL {
  ...
  constructor(private readonly llparse: LLParse,
    ...
    if (separateSpans) {
      table.set('schema', p.span(p.code.span('llsdp__on_url_schema')));
      table.set('host', p.span(p.code.span('llsdp__on_url_host')));
      table.set('path', p.span(p.code.span('llsdp__on_url_path')));
      table.set('query', p.span(p.code.span('llsdp__on_url_query')));
      table.set('fragment',
        p.span(p.code.span('llsdp__on_url_fragment')));
    } else {
      table.set('url', p.span(p.code.span('llsdp__on_url')));
    }
  }
  ...
  public build(): IURLResult {
    ...
    // Adaptors
    const toSDP = this.node('to_sdp');
    const toSDP09 = this.node('to_sdp_09');

    const skipToSDP = this.node('skip_to_sdp')
      .skipTo(toSDP);

    const skipToSDP09 = this.node('skip_to_sdp09')
      .skipTo(toSDP09);

    const skipCRLF = this.node('skip_lf_to_sdp09')
      .match('\r\n', toSDP09)
